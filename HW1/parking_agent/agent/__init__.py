...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        ''' 
        # position of agents
        string = "(at pt{}pt{} agent1)".format(self.state.agent.position.x, self.state.agent.position.y)

        # position of cars
        for car in self.state.cars:
            string +=  " (blocked pt{}pt{})".format(car.position.x, car.position.y)
        
        # relative position of grid cells
        width = self.width
        length = self.num_lanes

        for x in range(width):
            for y in range(length):
                current = "pt{}pt{}".format(x, y)
                new_x = x -1 if x else width-1
                forward_next = "pt{}pt{}".format(new_x, y)
                up_next = "pt{}pt{}".format(new_x, y-1) if y else "pt{}pt{}".format(new_x, y)
                down_next = "pt{}pt{}".format(new_x, y+1) if y < length-1 else "pt{}pt{}".format(new_x, y)
                string += " (forward_next {0} {1}) (up_next {0} {2}) (down_next {0} {3})".format(current, forward_next, up_next, down_next)
    ...
    def generateGoalString(self) :
        ...
        return "(at pt{}pt{} agent1)".format(x, y)    
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="FORWARD",
                  parameters=(("A", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell")),
                  precondition_string="(and (at ?pt1 ?A) (forward_next ?pt1 ?pt2) (not (blocked ?pt2)))",
                  effect_string="(and (at ?pt2 ?A) (not (at ?pt1 ?A)))")
