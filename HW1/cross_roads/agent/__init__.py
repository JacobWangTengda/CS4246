...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        '''
        width = self.width
        length = self.num_lanes

        # position of agents
        string = "(at pt{}pt{}pt0 agent1)".format(self.state.agent.position.x, self.state.agent.position.y)
        
        # position of cars: blocked positions
        for car in self.state.cars:
            car_x = car.position.x
            car_y = car.position.y
            speed = car.speed_range[0]

            string +=  " (blocked pt{}pt{}pt0)".format(car_x, car_y)
            ...
            while time < width:
                ...
                if car_x < 0:
                    car_x += width
                string +=  " (blocked pt{}pt{}pt{})".format(car_x, car_y, time)
                
                # trailing spot for cars moving at a speed less than -1
                for i in range(1, -1*speed):
                    trail = (car_x + i)%width
                    string += " (blocked pt{}pt{}pt{})".format(trail, car_y, time)
                
                time += 1

        # relative positions of grid cells
        for x in range(width):
            for y in range(length):
                for t in range(width-1):
                    current = "pt{}pt{}pt{}".format(x, y, t)
                    x1 = x -1 if x else width-1
                    x2 = x1 - 1 if x1 else width - 1
                    x3 = x2 - 1 if x2 else width -1
                    forward_next1 = "pt{}pt{}pt{}".format(x1, y, t+1)
                    forward_next2 = "pt{}pt{}pt{}".format(x2, y, t+1)
                    forward_next3 = "pt{}pt{}pt{}".format(x3, y, t+1)
                    up_next = "pt{}pt{}pt{}".format(x1, y-1, t+1) if y else "pt{}pt{}pt{}".format(x1, y, t+1)
                    down_next = "pt{}pt{}pt{}".format(x1, y+1, t+1) if y < length-1 else "pt{}pt{}pt{}".format(x1, y, t+1)
                    string += " (forward_next1 {0} {1}) (forward_next2 {0} {2}) (forward_next3 {0} {3}) (up_next {0} {4}) (down_next {0} {5})".format(current, forward_next1, forward_next2, forward_next3, up_next, down_next)
    ...
    def generateGoalString(self) :
        ...
        # 'or' statement is allowed in this version of PDDL
        string = "(or"
        x = self.state.finish_position.x
        y = self.state.finish_position.y

        for t in range(self.width):
            string += " (at pt{}pt{}pt{} agent1)".format(x, y, t)
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="UP",
                  parameters=(("A", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell")),
                  precondition_string="(and (at ?pt1 ?A) (up_next ?pt1 ?pt2) (not (blocked ?pt2)))",
                  effect_string="(and (at ?pt2 ?A) (not (at ?pt1 ?A)))")
    ...
    gen.addAction(name="FORWARD3",
                  parameters=(("A", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell")),
                  precondition_string="(and (at ?pt1 ?A) (forward_next3 ?pt1 ?pt2) (not (blocked ?pt2)))",
                  effect_string="(and (at ?pt2 ?A) (not (at ?pt1 ?A)))")
    
    gen.addAction(name="FORWARD2",
                  parameters=(("A", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell")),
                  precondition_string="(and (at ?pt1 ?A) (forward_next2 ?pt1 ?pt2) (not (blocked ?pt2)))",
                  effect_string="(and (at ?pt2 ?A) (not (at ?pt1 ?A)))")
    
    gen.addAction(name="FORWARD1",
                  parameters=(("A", "agent"), ("pt1", "gridcell"), ("pt2", "gridcell")),
                  precondition_string="(and (at ?pt1 ?A) (forward_next1 ?pt1 ?pt2) (not (blocked ?pt2)))",
                  effect_string="(and (at ?pt2 ?A) (not (at ?pt1 ?A)))")
